        ...
        <>
            ...
            <div className="fixed right-4 top-24 z-50 w-full max-w-xl rounded-3xl bg-white p-8 shadow-2xl">
                ...
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <p className="text-xl font-semibold text-gray-900">{cartItems.length} Projects</p>
                </div>
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        <>
                            ...
                            <div className="flex items-center justify-between rounded-2xl bg-gray-100 px-6 py-3">
                                <span className="text-sm font-medium text-gray-600">QF Round</span>
                                <span className="text-sm font-medium text-gray-900">Super duper round</span>
                            </div>

                            {/* Projects from first round */}
                            {cartItems.slice(0, Math.ceil(cartItems.length / 2)).map((project) => (
                                <div
                                    key={project.id}
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            ...
                                            className={`flex items-center justify-center transition-all duration-300 ease-in-out ${hoveredProjectId === project.id
                            ...
                            {cartItems.length > 1 && (
                                <>
                                    <div className="flex items-center justify-between rounded-2xl bg-gray-100 px-6 py-3">
                                        <span className="text-sm font-medium text-gray-600">QF Round</span>
                                        <span className="text-sm font-medium text-gray-900">The best round ever</span>
                                    </div>

                                    {/* Projects from second round */}
                                    {cartItems.slice(Math.ceil(cartItems.length / 2)).map((project) => (
                                        <div
                                            key={project.id}
                                            className="group relative flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 transition-all hover:border-gray-200"
                                        >
